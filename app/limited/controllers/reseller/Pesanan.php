...
class Pesanan extends reseller_controller
{
	...
	public function lihat($status = 'all') {
		...
		// cek $juragan ...
		$id_juragan = $this->config->item('id_default_reseller');
		...
		if(in_array($status, array('all', 'pending', 'terkirim')) && $juragan_ada && $cek ) {
			...
			$oleh = $this->pengguna->_id($user_slug);

			$query = $this->pesanan->ambil_semua(FALSE, FALSE, $status, $limit, $per_page, $cari, $oleh);

			$config['base_url'] = site_url('pesanan/' . $status);
			$config['total_rows'] = $this->pesanan->ambil_semua(FALSE, FALSE, $status, FALSE, FALSE, $cari, $oleh)->num_rows();
			...
			$this->load->view('reseller/header', $this->data);
			$this->load->view('reseller/pesanan/lihat', $this->data);
			$this->load->view('reseller/footer', $this->data);
		}
		else {
			show_404();
		}		
	...
	public function tambah() {
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->load->view('reseller/header', $this->data);
			$this->load->view('reseller/pesanan/tambah', $this->data);
			$this->load->view('reseller/footer', $this->data);
		}
		else {
