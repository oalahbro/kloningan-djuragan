...
class Faktur extends admin_controller {
	...
	public function json_keterangan($id_faktur) {
		...
		if ($keterangan !== NULL) {
			...
			// $html .= '<button class="btn btn-outline-primary dropdown-toggle btn-sm mb-2 mr-1" type="button" data-toggle="collapse" data-target="#keterangan-'.$id_faktur.'" aria-expanded="false" aria-controls="keterangan-'.$id_faktur.'">
		...
		if($pengiriman->num_rows() > 0) {
			// $html .= '<button class="btn btn-outline-dark dropdown-toggle btn-sm mb-2" type="button" data-toggle="collapse" data-target="#resi-'.$id_faktur.'" aria-expanded="false" aria-controls="resi-'.$id_faktur.'">
				//  <i class="fas fa-receipt"></i> Resi Kirim
			// </button>';

			// $html .= '<div class="collapse show" id="resi-'.$id_faktur.'"><div class="bg-light border border-dark p-1 rounded">
				// <h6>Pengiriman : </h6>
				// <ul class="ml-0 pl-3">';
		...
		if($gambar !== NULL) {
			...
			foreach ($gmb as $image) {
				// $html .= anchor($image, 'Custom Gambar ' . $i++, array('target' => '_blank', 'class' => 'dropdown-item'));
		...
		// if ($keterangan !== NULL) {
		// 	$html .= '<div class="collapse'.$clshow.'" id="keterangan-'.$id_faktur.'">'.nl2br($keterangan);
				
		// 	if($gambar !== NULL) {
		// 		$html .= '<div class="dropdown mt-3">';
		// 		$html .= '<button class="btn btn-outline-secondary btn-sm dropdown-toggle" type="button" id="customGambar' . $id_faktur .'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Gambar</button>';
		// 		$html .= '<div class="dropdown-menu" aria-labelledby="customGambar' . $id_faktur .'">';
		// 			$i = 1;
		// 			$gmb= json_decode($gambar);
		// 			foreach ($gmb as $image) {
		// 				$html .= anchor($image, 'Custom Gambar ' . $i++, array('target' => '_blank', 'class' => 'dropdown-item'));
	...
	public function json_pesanan($id_faktur) {
		// pesanan produk
