...
<?php if(is_login()) { $bulan_today = mdate("%Y-%m", now()); $date_today = mdate("%Y-%m-%d", now());?>
	...
	?>
		...
		function showValues() {
			...
			$( '#loader' ).load( '<?php echo site_url('admin/pesanan/load/'.$halaman .'/'.$juragan) ?>', 'submit=yes&juragan=<?php echo $juragan ?>&cari='+ cari).hide().fadeIn('slow');
		}
